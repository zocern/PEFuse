...
from model.select_network import define_G
...
class ModelPlain(ModelBase):
    ...
    def load(self):
        ...
        if load_path_G is not None:
            ...
            self.load_network(load_path_G, self.netG, strict=self.opt_train['G_param_strict'], param_key='params') #strict：控制加载时是否严格匹配模型参数。
    ...
    def define_loss(self):
        ...
        if G_lossfn_type == 'IVF':
            from model.loss import fusion_loss_ivf
            self.G_lossfn = fusion_loss_ivf().to(self.device)
        else:
