...
class ModelBase():
    def __init__(self, opt):
        self.opt = opt                         # opt
        self.save_dir = opt['path']['model']  # save model
    ...
    def model_to_device(self, network):
        """Model to device. It also warps model with DistributedDataParallel
        or DataParallel.
        Args:
            network (nn.Module)
        """
    ...
    def save_network(self, save_dir, network, network_label, iter_label):
        ...
        for key, param in state_dict.items():
            state_dict[key] = param.cpu()#将所有参数从 GPU 移到 CPU，确保在保存时不受 GPU 内存限制
